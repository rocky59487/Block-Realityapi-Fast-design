                    ...
                    if (island != null && island.getBlockCount() > 0) {
                        try {
                            ...
                            BlockPos center = new BlockPos(
                                ...
                                (min.getY() + max.getY()) / 2,
                                (min.getZ() + max.getZ()) / 2);
                            int radius = Math.max(Math.max(
                                max.getX() - min.getX(),
                                max.getY() - min.getY()),
                                max.getZ() - min.getZ()) / 2 + 2;

                            // 選擇分析所在的維度（遍歷所有已載入的 level）
                            ServerLevel targetLevel = null;
                            for (ServerLevel sl : srv3.getAllLevels()) {
                                if (sl.isLoaded(center)) {
                                    targetLevel = sl;
                                    break;
                                }
                            }
                            ...
                            int collapsed = CollapseManager.checkAndCollapse(targetLevel, center, radius);
                            if (collapsed > 0) {
                                LOGGER.info("[Physics] Island {} ({} blocks, r={}) — {} collapsed",
                                    sw.islandId(), island.getBlockCount(), radius, collapsed);
                            }
                        } catch (Exception e) {
