...
public record BeamElement(
    ...
    public double utilizationRatio(double axialForce, double moment, double shear) {
        ...
        // ★ audit-fix Y2-5: 線性交互公式（Eurocode EN 1993-1-1 §6.2.1）
        //   軸力與彎矩均為同軸正應力，在截面邊緣直接疊加：
        //     σ_edge = N/A + M/W
        //   因此正確的交互式: N/N_max + M/M_max ≤ 1.0
        //
        //   舊版 √(r²+r²) 是 L2-norm，對同軸應力偏不安全：
        //     axialRatio=0.6 + momentRatio=0.6:
        //     L2-norm = √(0.72) = 0.85 (判安全) ← 錯誤
        //     線性    = 1.2            (判破壞) ← 正確
        //
        //   剪力獨立檢查（Tresca 準則）— 與正應力不在同方向，取 max 正確。
        double normalCombined = axialRatio + momentRatio;
