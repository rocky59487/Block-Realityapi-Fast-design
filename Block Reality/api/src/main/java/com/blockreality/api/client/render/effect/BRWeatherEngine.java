...
public class BRWeatherEngine {
    ...
    private static WeatherType targetWeather = WeatherType.CLEAR;

    /** 當前天氣強度（0.0=無, 1.0=最大） */
    private static float weatherIntensity = 0.0f;
    ...
    private static float globalWetness = 0.0f;

    /** 積雪覆蓋度（0.0=無雪, 1.0=完全覆蓋） */
    private static float snowCoverage = 0.0f;

    /** 閃電冷卻計時器（tick） */
    private static int lightningCooldown = 0;

    /** 閃電螢幕閃光強度（快速衰減） */
    private static float lightningFlash = 0.0f;
