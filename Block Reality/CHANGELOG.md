...
- `PFSFTickPlanner` 與 M2 的 12+ plan opcode 就位;`nativeTickDbb()`
  進入點由 `NativePFSFBridge` 暴露,`NativePFSFRuntime` 目前僅用來跑
  `PFSFTickBoundaryTest` 的 scaffolding 驗證
...
- 新增 `PFSFTickBoundaryTest` 和 `CollapseEventParityTest` 確認現行
  逐原語路徑行為與 v0.3d 位元一致（CollapseManager 事件序列、
  SubscribeEvent 派發順序不變）
