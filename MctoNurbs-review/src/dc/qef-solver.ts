...
function jacobiEigen3x3(
  ...
  for (let i = 0; i < 50; i++) {
    ...
    let t;
    if (Math.abs(a[p][q]) < 1e-12 * Math.abs(diff)) {
      t = a[p][q] / diff;
    } else {
      const theta = 0.5 * diff / a[p][q];
    ...
    for (let j = 0; j < 3; j++) {
      if (j !== p && j !== q) {
        const apj = a[p][j];
        const aqj = a[q][j];
        a[p][j] = a[j][p] = apj - s * (aqj + apj * tau);
        a[q][j] = a[j][q] = aqj + s * (apj - aqj * tau);
      }

      const vpj = v[j][p];
      const vqj = v[j][q];
      v[j][p] = vpj - s * (vqj + vpj * tau);
      v[j][q] = vqj + s * (vpj - vqj * tau);
    }
  }
  ...
  const indices = [0, 1, 2];
  indices.sort((i, j) => Math.abs(eigenvalues[j]) - Math.abs(eigenvalues[i]));
